...
def _stringify_log_for_gemini(log: list) -> str:
    """
    Converts a list of log dictionaries into a string format suitable for the Gemini API.
...
def construct_chat_prompt(func):
    def wrapper(context, practice_lang, persona, user_profile, practice_lang_prof, desired_scenario):
        ...
        response = _query_chat(_stringify_log_for_gemini(context))
...
def construct_teacher_prompt(func):
    def wrapper(prompt, practice_lang, user_lang):
        ...
        response = _query_teacher(_stringify_log_for_gemini(context))
